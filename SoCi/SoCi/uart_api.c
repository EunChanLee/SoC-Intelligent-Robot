...
void uart1_buffer_read(unsigned char *buf, int size)
{
	...
	*/
	...
	int rx_len = uart_rx_level(UART1);

	if (rx_len >= size)
	{
		uart_read(UART1, &buf[0], size);
	}
}
...
int uart1_rxbuf_exist(void)
{
	return uart_rx_level(UART1);
}

void uart1_rxbuf_clear(void)
{
	int ret;
	ret = uart_rxbuf_clear(UART1);
	if (ret<0) {
		printf("Clearing buffer is failed!\n");
	}
}
